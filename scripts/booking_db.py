...
def upsert_booking(conn, row: Dict[str, Any]) -> bool:
    """Insert or update a single booking. Returns True if successful."""
...
def upsert_bookings_batch(conn, rows: List[Dict[str, Any]]) -> int:
    """Insert or update multiple bookings. Returns count of rows processed."""
    count = 0
    for r in rows:
        if upsert_booking(conn, r):
            count += 1
    return count
